-- OurAI initial schema (all objects live in schema `ourai`).
...
create table ourai.company_members (
...
create index ideas_company_status_idx on ourai.ideas (company_id, status);

-- A promoted idea → one branch → one agent session.
create table ourai.work_items (
  id           uuid primary key default extensions.gen_random_uuid(),
  company_id   uuid not null references ourai.companies (id) on delete cascade,
  idea_id      uuid references ourai.ideas (id) on delete set null,
  title        text not null,
...
create unique index work_items_company_branch_uidx on ourai.work_items (company_id, branch);
create index work_items_company_status_idx on ourai.work_items (company_id, status);

-- One agent run bound to a work item + its branch.
create table ourai.sessions (
  id             uuid primary key default extensions.gen_random_uuid(),
  work_item_id   uuid not null references ourai.work_items (id) on delete cascade,
...
create index sessions_work_item_idx on ourai.sessions (work_item_id);
...
create table ourai.events (
  id          uuid primary key default extensions.gen_random_uuid(),
  session_id  uuid not null references ourai.sessions (id) on delete cascade,
  seq         bigint not null,
...
create unique index events_session_seq_uidx on ourai.events (session_id, seq);
create index events_session_created_idx on ourai.events (session_id, created_at);

-- Assign a gap-free per-session sequence at insert time.
create or replace function ourai.assign_event_seq() returns trigger as $$
begin
  if new.seq is null or new.seq = 0 then
    select coalesce(max(seq), 0) + 1 into new.seq from ourai.events where session_id = new.session_id;
  end if;
  return new;
end;
...
create trigger events_assign_seq
  before insert on ourai.events
  for each row
  execute function ourai.assign_event_seq();
